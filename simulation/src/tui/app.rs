...
use simulation::{Query, QueryStatus, ReversibleProgression, Simulation, Status};
...
mod test {
    ...
    proptest! {
        ...
        fn test_random_selector(sequence in prop::collection::vec(prop::bool::ANY, 100)) {
            ...
            let config_file = File::open(config_path.join("tests").join("config.yml")).unwrap();
            let queries_file = File::open(config_path.join("tests").join("queries.jl")).unwrap();
            let sim = Simulation::new(
                &Config::from_yaml(config_file).unwrap(),
                serde_json::Deserializer::from_reader(queries_file)
                    .into_iter()
